...
export function KonvaSlideEditor({ slide, editable, command, onStateChange, onSave }: KonvaSlideEditorProps) {
  ...
  const availableWidth = Math.max(1, containerSize.width || 1200);
  const availableHeight = Math.max(1, containerSize.height || 675);
  const scale = Math.min(availableWidth / parsed.width, availableHeight / parsed.height, 1.15);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      ...
      setContainerSize((current) => {
        const next = {
          width: Math.max(1, Math.floor(rect.width || current.width)),
          height: Math.max(1, Math.floor(rect.height || current.height)),
        };
        if (Math.abs(next.width - current.width) < 2 && Math.abs(next.height - current.height) < 2) return current;
        return next;
      });
    });
  ...
  const addImageFromFile = (file: File, position?: { x: number; y: number }) => {
  ...
  return (
    <div className={`konva-slide-editor ${editable ? "konva-slide-editor-editable" : "konva-slide-editor-readonly"}`} ref={containerRef}>
      ...
      />
      <div
        className="konva-stage-shell"
        style={{
          width: stageWidth,
          height: stageHeight,
          backgroundImage: `url(${svgToDataUrl(backgroundSvgForRender)})`,
        ...
        onDrop={(event) => {
          ...
          addImageFromFile(file, {
            x: Math.max(0, (event.clientX - rect.left) / scale - 180),
            y: Math.max(0, (event.clientY - rect.top) / scale - 110),
          });
        }}
      >
        <Stage
  ...
  if (node.type === "rect") {
    return (
      <div className="konva-floating-tools" style={{ left, top }}>
        ...
        <button title={t("editor.delete")} type="button" onClick={onDelete}><Trash2 size={14} /></button>
      </div>
    );
  }
  if (node.type === "image") {
    return (
      <div className="konva-floating-tools" style={{ left, top }}>
        ...
        <button title={t("editor.delete")} type="button" onClick={onDelete}><Trash2 size={14} /> {t("editor.delete")}</button>
      </div>
    );
  }
  if (node.type === "table") {
...
function documentToParsedSlide(document: SlideDocument, fallbackSvg: string): ParsedSlide {
  ...
  const backgroundSvg = document.backgroundSvg?.trim() || fallbackSvg?.trim() || blankSvg();
  const documentElements = Array.isArray(document.elements) ? document.elements : [];
  const documentNodes = documentElements.map(normalizeDocumentNode).filter((node): node is EditorNode => Boolean(node));
