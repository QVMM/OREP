...
async def _enrich_web_search(
    ...
    if provider == "mimo" and not (settings.mimo_api_key or "").strip():
        stats.web_error = "no_api_key"
        ctx.queries_used.append("web: <skipped: no MiMo API key>")
        return
    ...
    try:
        ...
        if provider == "mimo":
            ...
            results = await _mimo_web_search(query, config.max_results_per_source)
...
async def _mimo_web_search(query: str, max_results: int) -> list[dict[str, str]]:
    ...
    base_url = normalize_openai_base_url(settings.mimo_base_url)
    client = AsyncOpenAI(api_key=settings.mimo_api_key, base_url=base_url)
    prompt = (
    ...
    response = await client.chat.completions.create(
        model=settings.mimo_model or "mimo-v2.5-pro",
