...
function getProviderDefaults(
  providers: { name: string; default_base_url?: string | null; models?: { id: string }[] }[],
  providerName: string,
) {
...
function getBrowserSafeRoutingProfile(
  providerName: string,
  saved: RoutingProfile | undefined,
  ...
  if (providerName === "mimo") {
    return {
      ...
      apiKey: "",
      deepseekSettings: saved?.deepseekSettings,
      openaiSettings: saved?.openaiSettings,
    };
  }
  return {
    model: saved?.model || defaults.model,
...
export function GeneratePage() {
  ...
  const [researchConfig, setResearchConfig] = useState<ResearchConfig>(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...base,
          web_search_provider: base.web_search_provider || (parsed.web_search_provider as "mimo" | "tavily" | "serpapi" | undefined) || undefined,
  ...
  useEffect(() => {
    if (!provider && providers.length > 0) {
      ...
      const safeProfile = getBrowserSafeRoutingProfile(defaultProvider, saved, defaults);
      setProvider(defaultProvider);
      setModel(safeProfile.model);
      setBaseUrl(safeProfile.baseUrl);
      setApiKey(safeProfile.apiKey);
      setDeepSeekSettings(safeProfile.deepseekSettings ?? DEFAULT_DEEPSEEK_SETTINGS);
      setOpenAISettings(safeProfile.openaiSettings ?? DEFAULT_OPENAI_SETTINGS);
    }
  }, [provider, providers]);
  ...
  useEffect(() => {
    ...
    const safeProfile = getBrowserSafeRoutingProfile(provider, saved, defaults);
    setModel(safeProfile.model);
    setBaseUrl(safeProfile.baseUrl);
    setApiKey(safeProfile.apiKey);
    setDeepSeekSettings(safeProfile.deepseekSettings ?? DEFAULT_DEEPSEEK_SETTINGS);
  ...
  useEffect(() => {
    ...
    profiles[provider] = {
      ...
      baseUrl: provider === "mimo" ? defaults.baseUrl : baseUrl,
      apiKey: provider === "mimo" ? "" : apiKey,
  ...
  }, [apiKey, baseUrl, deepSeekSettings, model, openAISettings, provider, providers, targetJobId]);
  ...
  useEffect(() => {
    try {
      ...
      const next = {
        web_search_provider: researchConfig.web_search_provider || existing.web_search_provider || "mimo",
  ...
  const launchGeneration = async () => {
    ...
    const defaults = getProviderDefaults(providers, provider);
    const requestBaseUrl = provider === "mimo" ? defaults.baseUrl : baseUrl;
    const requestApiKey = provider === "mimo" ? "" : apiKey;
    const profiles = readRoutingProfiles();
    profiles[provider] = {
    ...
    const nextJobId = await startGeneration({
      ...
      model_config: {
        ...
        base_url: requestBaseUrl || undefined,
