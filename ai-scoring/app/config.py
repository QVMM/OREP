...
class Settings:
    ...
    LLM_EVIDENCE_MEMORY_RETRY_MAX_TOKENS: int = max(
        2000, int(os.getenv("LLM_EVIDENCE_MEMORY_RETRY_MAX_TOKENS", "4000"))
    )
    LLM_EVIDENCE_INCOMPLETE_MIN_FAILED_WINDOWS: int = max(
        1, int(os.getenv("LLM_EVIDENCE_INCOMPLETE_MIN_FAILED_WINDOWS", "2"))
    )
    ASR_TRANSCRIPT_ENGINE_ID: str = os.getenv(
        "ASR_TRANSCRIPT_ENGINE_ID", "fun-asr-realtime-2026-02-28"
    )
    ASR_CACHE_WAIT_SECONDS: int = max(0, int(os.getenv("ASR_CACHE_WAIT_SECONDS", "900")))
    ...
    VIDEO_BATCH_SIZE: int = int(os.getenv("VIDEO_BATCH_SIZE", "4"))  # qwen3-vl-flash 8帧+长提示会超时
    VIDEO_ANALYSIS_MAX_WORKERS: int = max(1, int(os.getenv("VIDEO_ANALYSIS_MAX_WORKERS", "2")))
    VIDEO_ANALYSIS_TIMEOUT_SECONDS: float = max(
        15.0, float(os.getenv("VIDEO_ANALYSIS_TIMEOUT_SECONDS", "90"))
    )
    VIDEO_ANALYSIS_MAX_RETRIES: int = max(0, int(os.getenv("VIDEO_ANALYSIS_MAX_RETRIES", "1")))
    VIDEO_ANALYSIS_TIMEOUT_STREAK_ABORT: int = max(
        2, int(os.getenv("VIDEO_ANALYSIS_TIMEOUT_STREAK_ABORT", "6"))
    )
