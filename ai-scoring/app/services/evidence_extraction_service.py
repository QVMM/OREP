...
# 缺证地板：无红线时 E0 给满证据帽（k=1.0）；仅「真红线」才允许 final=0
E0_MISSING_EVIDENCE_FLOOR_RATIO = 1.0
LOW_CONFIDENCE_FLOOR_THRESHOLD = 0.6
_HARD_REDLINE_REASON = re.compile(
    r"(伪造成果|伪造|明确侵权|侵权盗用|隐私泄露|越权服务|诚信红线|"
    r"人身(?:安全)?风险|公共安全|未处置.*(?:风险|危害)|"
    r"关键工作明显由外部人员完成|"
    r"成员说法互相矛盾且影响任务真实性)",
    re.I,
)
_WEAK_SYSTEM_EVIDENCE_REASON = re.compile(
    r"(本轮记忆中证据有限|记忆中证据有限|session\s*memory|证据记忆不足)",
    re.I,
)
_WEAK_PERFORMANCE_SIGNAL = re.compile(
    r"(分工|负责(?:模块|部分|这块)?|协作|联调|交接|补台|队员|同事|"
    r"权限|脱敏|备份|开源|授权|依赖清单|安全测试|合规|隐私|密钥|登录|"
    r"缺陷|迭代|优化前后|版本记录|测试覆盖)",
    re.I,
)
# 分维度弱信号（用于自动绑证 / 升 E1）
_WEAK_SIGNAL_BY_DIMENSION: dict[str, re.Pattern[str]] = {
    "professionalism": re.compile(
        r"(权限|脱敏|备份|开源|授权|依赖|合规|隐私|密钥|登录|安全测试|职业道德|贡献)",
        re.I,
    ),
    "teamwork": re.compile(
        r"(分工|负责(?:模块|部分|这块)?|协作|联调|交接|补台|队员|同事|看板|成员|角色)",
        re.I,
    ),
    "skill_level": re.compile(
        r"(缺陷|迭代|优化前后|版本记录|测试覆盖|调试|配置|异常处理|IDE|代码)",
        re.I,
    ),
    "application_value": re.compile(
        r"(用户|落地|成本|效率|场景|试点|反馈|价值)",
        re.I,
    ),
    "innovation": re.compile(
        r"(创新|新颖|改进|对比|基线|指标|成效)",
        re.I,
    ),
}
PILOT_RULE_FILES = {
...
def extract_evidence_from_llm_payload(content: str, track_rule: dict[str, Any]) -> dict[str, Any]:
    ...
    for observation in track_rule.get("observations") or []:
        item, item_warnings, synthetic = _normalize_observation_evidence(
            observation,
        ...
        if synthetic and isinstance(synthetic, dict):
            sid = str(synthetic.get("evidenceId") or "")
            if sid:
                evidence_items_by_id[sid] = synthetic
                if _has_evidence_payload(synthetic):
                    evidence_ids.add(sid)
                # 写入库存（避免因 normalize 内已 put map 而漏 append）
                if not any(
                    str(e.get("evidenceId")) == sid
                    for e in normalized["evidenceItems"]
                    if isinstance(e, dict)
                ):
                    normalized["evidenceItems"].append(synthetic)
    ...
    # 仅统计「非系统合成」证据库存是否完全未绑定，避免弱信号合成误触发 unbound
    real_inventory_ids = {
        str(item.get("evidenceId"))
        for item in normalized["evidenceItems"]
        if item.get("evidenceId") is not None
        and not item.get("synthetic")
        and _has_evidence_payload(item)
    }
    if real_inventory_ids and not any(
        any(str(eid) in real_inventory_ids for eid in (item.get("evidenceIds") or []))
...
def _normalize_observation_evidence(
    ...
    dimension_code = str(rule_observation.get("dimensionCode") or "")
    item = model_item or {}
    ...
    signal_blob = " ".join(
        [
            raw_performance_reason,
            str(item.get("evidenceReason") or ""),
            str(item.get("supportSummary") or ""),
            str(item.get("modelReason") or ""),
        ]
    )
    has_weak_signal = _has_weak_signal(dimension_code, signal_blob)

    # 弱信号：优先匹配库存证据；否则合成可绑定的弱证据，才能合法升到 E1
    if not evidence_ids and has_weak_signal:
        matched = _match_inventory_for_weak_signal(
            dimension_code,
            evidence_items_by_id or {},
            valid_evidence_ids,
        )
        if matched:
            evidence_ids = matched
            warnings.append(f"{code}:weak_signal_bound_inventory")
        else:
            synthetic_evidence = _build_synthetic_weak_evidence(
                observation_code=str(code or "OXX"),
                dimension_code=dimension_code,
                text=signal_blob,
            )
            if synthetic_evidence and _has_evidence_payload(synthetic_evidence):
                sid = str(synthetic_evidence["evidenceId"])
                evidence_ids = [sid]
                # 同步进本轮 valid 集合，后续 hard-gate / 绑定检查可见
                valid_evidence_ids.add(sid)
                if evidence_items_by_id is not None:
                    evidence_items_by_id[sid] = synthetic_evidence
                warnings.append(f"{code}:weak_signal_synthetic_evidence")
    ...
    # 弱信号且已绑证：至少 E1（不主动抬到 E2+）
    if has_weak_signal and evidence_ids and level == "E0":
        level = "E1"
        warnings.append(f"{code}:weak_signal_forced_E1")
    ...
    # 始终输出绝对分；兼容规则文件里「比例帽」(E5=1.0) 与「绝对分帽」(E5=maxScore)
    score_cap = _resolve_evidence_cap_points(evidence_caps, level, max_score)
    ...
    if _WEAK_SYSTEM_EVIDENCE_REASON.search(evidence_reason) or _WEAK_SYSTEM_EVIDENCE_REASON.search(
        raw_performance_reason
    ):
        # 禁止把系统记忆不足当成学生零分理由
        evidence_reason = (
            "可核验证据不足，按当前证据等级上限计分；请补定位材料后上浮"
            if level == "E0"
            else "弱信号已计入 E1 上限；请补更完整可复核材料后上浮"
        )
        warnings.append(f"{code}:weak_system_evidence_reason_rewritten")
    ...
    if _is_external_evidence_only_performance_reason(raw_performance_reason):
        ...
        # 弱阳性：理由里出现分工/合规等信号时，勿把表现直接打成 0
        if performance_level == "not_demonstrated" and has_weak_signal:
            performance_level = "partial"
            performance_ratio = PERFORMANCE_RATIOS["partial"]
            performance_reason = (
                (raw_performance_reason + "；" if raw_performance_reason else "")
                + (
                    "检测到弱现场信号并已绑定证据，表现按部分达成计，受 E1 证据上限约束"
                    if level != "E0"
                    else "检测到弱现场信号，表现按部分达成计，仍受证据等级上限约束"
                )
            )
            performance_assessment_status = (
                "weak_signal_e1" if level != "E0" else "weak_signal_partial"
            )
            warnings.append(f"{code}:weak_performance_signal_partial")

    base_score = round(max_score * performance_ratio, 2)
    confidence = _clamp(
        _safe_float(item.get("confidence"), DEFAULT_CONFIDENCE_BY_EVIDENCE_LEVEL.get(level, 0)),
        0,
        1,
    )
    redline = _is_hard_redline(
        rule_observation=rule_observation,
        performance_reason=performance_reason,
        evidence_reason=evidence_reason,
        support_summary=str(item.get("supportSummary") or ""),
        model_reason=str(item.get("modelReason") or ""),
    )
    # P0：缺证地板 — E0 且非红线时，final 至少落到 E0 证据帽（表现 0 不再整项清零）
    if _should_apply_e0_missing_floor(
        level=level,
        redline=redline,
        ...
        floor = round(score_cap * E0_MISSING_EVIDENCE_FLOOR_RATIO, 2)
        if base_score < floor:
            base_score = floor
            performance_assessment_status = "e0_missing_evidence_floor"
            if performance_level == "not_demonstrated":
                performance_reason = (
                    "本观测点缺少可核验证据，按 E0 证据上限计地板分（非红线零分）；"
                    "补齐可定位证据后可上浮。"
                )
            warnings.append(f"{code}:e0_missing_evidence_floor")
    if redline and level == "E0":
        # 真红线：允许保持 0，并标注
        performance_assessment_status = "hard_redline_zero"
        warnings.append(f"{code}:hard_redline_zero_allowed")

    validity = item.get("validityStatus") or ("missing_evidence" if level == "E0" else "valid")
    if level != "E0" and validity == "missing_evidence":
        validity = "valid"
    return {
    ...
    }, warnings, synthetic_evidence


def _has_weak_signal(dimension_code: str, text: str) -> bool:
    """是否出现可用的弱阳性信号。

    纯「未提及/缺少 XXX」否定句不计入，避免把缺证叙述误升为 E1。
    """
    if not text:
        return False
    # 去掉明显否定片段后再匹配
    scrubbed = re.sub(
        r"(未提及|未展示|未说明|没有|无任何|完全没有|缺少|缺乏|无法识别)[^。；;\n]{0,40}",
        " ",
        text,
    )
    dim_pat = _WEAK_SIGNAL_BY_DIMENSION.get(str(dimension_code or ""))
    if dim_pat and dim_pat.search(scrubbed):
        return True
    return bool(_WEAK_PERFORMANCE_SIGNAL.search(scrubbed))


def _match_inventory_for_weak_signal(
    dimension_code: str,
    ...
    *,
    ...
    """从已有证据库存里按维度关键词匹配可绑定 ID。"""
    pat = _WEAK_SIGNAL_BY_DIMENSION.get(str(dimension_code or "")) or _WEAK_PERFORMANCE_SIGNAL
    hits: list[tuple[int, str]] = []
    for eid in valid_evidence_ids:
        row = evidence_items_by_id.get(str(eid)) or {}
        if row.get("synthetic"):
            continue
        blob = f"{row.get('text') or ''} {row.get('sourceRef') or ''}"
        if not pat.search(blob):
            continue
        # 更长匹配略优先
        hits.append((len(blob), str(eid)))
    hits.sort(reverse=True)
    return [eid for _, eid in hits[:limit]]


def _build_synthetic_weak_evidence(
    *,
    ...
    if len(cleaned) < 4:
        cleaned = f"观测点 {observation_code} 存在弱现场信号（口头/过程提示），待补强可核验材料"
    excerpt = cleaned[:220]
    return {
        "evidenceId": f"sys-weak-{observation_code}",
        "sourceType": "system_weak_signal",
        "sourceRef": f"weak-signal@{observation_code}",
        "text": excerpt,
        "confidence": 0.35,
        "synthetic": True,
        "dimensionCode": dimension_code,
        "note": "weak_signal_auto_bound_for_E1",
    }


def _resolve_evidence_cap_points(
    evidence_caps: dict[str, Any] | None,
    level: str,
    max_score: float,
) -> float:
    """把规则里的 evidenceCaps 解析为绝对分。

    - 比例方案：E5≈1.0 且各级 ≤1 → cap = ratio * maxScore
    - 绝对分方案：E5≈maxScore 或存在 >1 的帽 → 直接用点数
    """
    caps = evidence_caps if isinstance(evidence_caps, dict) else {}
    raw = _safe_float(caps.get(level), 0.0)
    if max_score <= 0:
        return max(0.0, raw)
    e5 = _safe_float(caps.get("E5"), 0.0)
    values = [_safe_float(v, 0.0) for v in caps.values()]
    looks_like_ratio = e5 <= 1.0 + 1e-9 and all(v <= 1.0 + 1e-9 for v in values)
    if looks_like_ratio:
        return round(min(max_score, max(0.0, raw * max_score)), 2)
    return round(min(max_score, max(0.0, raw)), 2)


def _is_hard_redline(
    *,
    rule_observation: dict[str, Any],
    ...
    """仅真红线可零分；『无关/未展示』类不走红线。"""
    blob = " ".join(
        [
            performance_reason or "",
            evidence_reason or "",
            support_summary or "",
            model_reason or "",
        ]
    )
    if _HARD_REDLINE_REASON.search(blob):
        return True
    for trigger in rule_observation.get("zeroScoreTriggers") or []:
        t = str(trigger or "")
        # 规则模板里的「无关且无证据」只是 E0，不是红线
        if "无关" in t and "有效证据" in t:
            continue
        if "完全缺失" in t or "完全没有" in t:
            continue
        if _HARD_REDLINE_REASON.search(t) and any(
            key and key in blob for key in re.findall(r"[\u4e00-\u9fff]{2,}", t)[:6]
        ):
            return True
    return False


def _should_apply_e0_missing_floor(
    *,
    level: str,
    redline: bool,
    base_score: float,
    ...
    if redline:
        return False
    if str(level or "").upper() != "E0":
        return False
    if score_cap <= 0:
        return False
    # 表现已达/超过帽则不必抬
    if base_score >= score_cap - 1e-9:
        return False
    validity_l = str(validity or "").lower()
    if validity_l in {"missing_evidence", "invalid", ""} or confidence < LOW_CONFIDENCE_FLOOR_THRESHOLD:
        return True
    # 默认 E0 都给地板，避免「无证据=零能力」
    return True
