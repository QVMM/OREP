...
class LaTeXParser(PaperParser):
    ...
    async def _parse_zip(
        ...
        if not main_tex:
            from .project_zip_parser import ProjectZipParser

            logger.info(
                "No main .tex file found in %s; parsing as project material ZIP.",
                zip_path,
            )
            parser = ProjectZipParser()
            paper = await parser.parse(extract_dir, output_dir)
            LaTeXParser.last_parse_info = dict(ProjectZipParser.last_parse_info)
            LaTeXParser.last_parse_info["path"] = "project_zip_fallback"
            return paper

        paper = await self._parse_tex(main_tex, output_dir, images_dir)
        LaTeXParser.last_parse_info = {"path": "latex", "main_tex": str(main_tex)}
        return paper
