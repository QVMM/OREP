...
from backend.generator.svg_finalize.embed_images import build_image_index
...
async def get_critic_history(job_id: str, request: Request) -> dict:
...
async def get_critic_archive_svg(job_id: str, filename: str, request: Request) -> Response:
...
async def get_preview(job_id: str, request: Request, render_ready: bool = False) -> PreviewResponse:
    job = session_manager.get_job(job_id)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found.")
    ensure_job_owner(job, request)
    ...
    return await _build_preview_response(
        job_id,
        Path(job.project_dir) if job.project_dir else None,
        job.output_path,
        normalized_status,
...
async def update_preview_slide(job_id: str, slide_index: int, payload: SlideUpdateRequest, request: Request) -> PreviewSlide:
    job = session_manager.get_job(job_id)
    ensure_job_owner(job, request)
...
async def create_preview_slide(job_id: str, payload: SlideCreateRequest, request: Request) -> PreviewSlide:
    job = session_manager.get_job(job_id)
    ensure_job_owner(job, request)
    project_dir = _editable_project_dir(job)
    slide_files, source = _editable_slide_files(project_dir)
...
async def delete_preview_slide(job_id: str, slide_index: int, request: Request) -> PreviewResponse:
    job = session_manager.get_job(job_id)
    ensure_job_owner(job, request)
    project_dir = _editable_project_dir(job)
...
async def get_project_preview(project_dir: str, request: Request) -> PreviewResponse:
    ...
    for job in session_manager.list_jobs():
        if job.project_dir and _resolve_workspace_path(job.project_dir) == resolved_project_dir:
            matched_job = job
            break
...
async def _build_preview_response(
    ...
    if project_dir and project_dir.exists():
        ...
        image_index = await aoffload(build_image_index, project_dir) if render_ready else None
        for index, svg_path in enumerate(slide_files, start=1):
            if render_ready:
                # ``prepare_svg_file_for_render`` rewrites href base64 inlines etc.
                # It is export-grade work and can be very expensive across dozens
                # of historical slides, so preview uses raw SVG by default.
                try:
                    prepared_path = await aoffload(prepare_svg_file_for_render, svg_path, image_index)
            ...
            slides.append(
                PreviewSlide(
                    ...
                    source=source,
                    content=content,
                    notes=_read_slide_notes(project_dir, svg_path, index),
...
def _editable_project_dir(job) -> Path:
    ...
    if job.status not in {"complete", "error", "cancelled"}:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Only finished presentations can be edited.")
...
def _read_slide_notes(project_dir: Path, svg_path: Path, slide_index: int | None = None) -> str:
    artifact_notes = _read_script_skill_notes(project_dir, slide_index)
    path = _slide_notes_path(project_dir, svg_path)
    if not path.exists():
        return artifact_notes
    try:
        notes = path.read_text(encoding="utf-8")
    except OSError:
        return artifact_notes
    if artifact_notes and _looks_like_weak_slide_notes(notes):
        return artifact_notes
    if not artifact_notes and _looks_like_legacy_template_notes(notes):
        return ""
    return notes


def _read_script_skill_notes(project_dir: Path, slide_index: int | None) -> str:
    if not slide_index:
        return ""
    path = project_dir / "roadshow_script_skill.json"
    if not path.exists():
        return ""
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        ...
        return ""
    if data.get("mode") == "deterministic_fallback":
        return ""
    ...
    for page in pages:
        ...
        try:
            ...
            continue
        if page_index == slide_index:
            script = page.get("script")
            return script[:4000] if isinstance(script, str) else ""
    return ""


def _looks_like_weak_slide_notes(notes: str) -> bool:
    text = re.sub(r"\s+", "", notes or "")
    if not text:
        return True
    if len(text) < 220:
        return True
    return bool(re.search(r"也就是说|这里我们重点说明|首先[，,].{0,60}其次[，,]|职业院校技能大赛作品汇报", text))


def _looks_like_legacy_template_notes(notes: str) -> bool:
    text = re.sub(r"\s+", "", notes or "")
    if not text:
        return False
    patterns = [
    ...
    return hits >= 2 or bool(re.search(r"首先[，,]国家政策明确支持.*其次[，,]传统.*同时[，,]项目", text))
...
def _write_slide_notes(project_dir: Path, svg_path: Path, notes: str) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    if notes.strip():
        path.write_text(notes[:4000], encoding="utf-8")
    else:
...
def _normalize_slide_document(document: dict[str, Any] | None, content: str) -> dict[str, Any] | None:
    ...
    if isinstance(notes, str):
        normalized["speakerNotes"] = notes[:4000]
    return normalized
