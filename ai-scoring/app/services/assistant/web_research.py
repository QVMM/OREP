...
_FILE_EXT_RE = re.compile(r"\.(pdf|xlsx?|docx?|zip|rar)(\?|$)", re.I)
...
def extract_page_structure(html: str, base_url: str) -> dict[str, Any]:
    """从 HTML 抽结构化字段：文号、日期、附件表（不依赖站点写死）。"""
    text_blob = _WS.sub(" ", _TAG.sub(" ", unescape(html or "")))
    doc_no = ""
    # 常见公文文号形态：教职成函〔2026〕1号 / 豫教〔2025〕12号
    m = re.search(
        r"([\u4e00-\u9fff]{1,12}(?:函|发|字)?〔\d{4}〕\s*\d+\s*号)",
        text_blob,
    )
    if m:
        doc_no = m.group(1).replace(" ", "")
    date = ""
    m = re.search(r"(20\d{2}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日)", text_blob)
    if m:
        date = re.sub(r"\s+", "", m.group(1))
    if not date:
        m = re.search(r"(20\d{2}-\d{1,2}-\d{1,2})", text_blob)
        if m:
            date = m.group(1)
    attachments = _extract_file_attachments(html, base_url)
    return {
        "docNo": doc_no,
        "date": date,
...
def _extract_file_attachments(html: str, base_url: str, *, max_items: int = 12) -> list[dict[str, str]]:
    """从 HTML 抽出可下载文件链接（pdf/xlsx/doc…），相对路径按 base_url 拼绝对地址。"""
    if not html:
        return []
    ...
    for m in _A_HREF_RE.finditer(html):
        ...
        label = _WS.sub(" ", _TAG.sub(" ", unescape(m.group(2) or ""))).strip()
        ...
        if not _FILE_EXT_RE.search(abs_url):
            # 有的站点用下载接口不带扩展名，但文案含「附件/名单」且 href 含 W0xxxx 类文件名
            if not (
                re.search(r"(附件|名单|下载)", label)
                and re.search(r"W0\d{10,}|\.pdf|\.xls", href, re.I)
            ):
                continue
        # 去跟踪参数噪音，保留路径；政府站尽量 https
        key = abs_url.split("#")[0]
        if key.startswith("http://") and any(
            d in key for d in ("moe.gov.cn", "gov.cn", "edu.cn", "eol.cn")
        ):
            key = "https://" + key[len("http://") :]
        if key in seen:
            continue
        seen.add(key)
        title = label if label and len(label) >= 4 else key.rsplit("/", 1)[-1]
        title = title[:100]
        out.append({"title": title, "url": key})
...
def fetch_page(url: str, *, max_chars: int = 1800, timeout: float = 8.0) -> dict[str, Any]:
    ...
    try:
        ...
        structure = extract_page_structure(html, final_url)
        attachments = structure.get("attachments") or []
        title, text = _strip_html_to_text(html)
        # 有附件时正文可稍短也算成功（通知页常正文不长）
        if len(text) < 40 and not attachments:
            return {"ok": False, "url": final_url, "title": title, "error": "too_short"}
        # 去掉开头重复标题
        if title and text.startswith(title):
            text = text[len(title) :].strip()
        meta_bits = []
        if structure.get("docNo"):
            meta_bits.append(f"文号：{structure['docNo']}")
        if structure.get("date"):
            meta_bits.append(f"日期：{structure['date']}")
        # 把附件清单追加进 text，供模型直接引用完整 URL
        if attachments:
            lines = ["可下载附件（完整 URL，请原样给出 Markdown 链接，本对话不会上传文件）："]
            for i, a in enumerate(attachments[:10], 1):
                lines.append(f"{i}. {a['title']} → {a['url']}")
            att_block = "\n".join(lines)
            budget = max(400, max_chars - len(att_block) - 40)
            text = (text[:budget] + ("…" if len(text) > budget else "")).rstrip()
            if meta_bits:
                text = "；".join(meta_bits) + "\n" + text
            text = text + "\n\n" + att_block
        else:
    ...
    for h in hits:
        ...
        if page and page.get("ok"):
            ...
            if isinstance(atts, list) and atts:
                ...
                # 摘要里露出前几条，便于前端「依据」卡片一眼看到可下载
                att_names = [
                    str(a.get("title") or "")[:40]
                    for a in atts[:3]
                    if isinstance(a, dict)
                ]
                if att_names:
                    item["snippet"] = (
                        "可下载：" + "；".join(att_names) + " | " + str(item.get("snippet") or "")
                    )[:220]
