...
FIRST_BATCH_NEW_RULES = [
    "track_26_electronics_integrated_circuit_v1_2_engine_draft.json",
    "track_14_smart_equipment_v1_2_engine_draft.json",
    "track_12_mechanical_design_manufacturing_v1_2_engine_draft.json",
    "track_13_mechatronic_installation_maintenance_v1_2_engine_draft.json",
]
...
def test_first_batch_new_track_rules_are_promoted_to_pilot_but_not_active():
    for file_name in FIRST_BATCH_NEW_RULES:
        ...
        assert rule["qualityGates"]["requiresRuleAdmissionAudit"] is True
        assert rule["qualityGates"]["requiresQualityReport"] is True
...
def test_first_batch_rule_hashes_match_canonical_payloads():
    for file_name in FIRST_BATCH_NEW_RULES:
        rule = load_rule(file_name)
...
def test_first_batch_rules_pass_candidate_gate_when_samples_audit_and_report_are_ready():
...
def test_first_batch_plan_entries_reference_generated_draft_rules():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    planned_tracks = {
        track["trackId"]: track
        for batch in plan["batches"]
        for track in batch["tracks"]
        if track["trackId"] in {"26", "14", "12", "13"}
    }

    assert set(planned_tracks) == {"26", "14", "12", "13"}
    for track in planned_tracks.values():
        rule_file = track["structuredRuleFile"]
        assert track["status"] == "pilot_generated"
        assert track["structuredRuleStatus"] == "pilot_generated"
        assert (Path(__file__).resolve().parents[2] / rule_file).exists()
