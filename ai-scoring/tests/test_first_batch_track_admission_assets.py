...
FIRST_BATCH_TRACKS = {
    "26": "track_26_electronics_integrated_circuit_v1_2_engine_draft.json",
    "14": "track_14_smart_equipment_v1_2_engine_draft.json",
    "12": "track_12_mechanical_design_manufacturing_v1_2_engine_draft.json",
    "13": "track_13_mechatronic_installation_maintenance_v1_2_engine_draft.json",
}
...
def test_first_batch_admission_assets_exist_and_meet_sample_mix_gate():
    for track_id in FIRST_BATCH_TRACKS:
...
def test_first_batch_admission_assets_pass_candidate_gate():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    for track_id, rule_file in FIRST_BATCH_TRACKS.items():
...
def test_first_batch_plan_entries_reference_admission_assets():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    planned_tracks = {
        track["trackId"]: track
        for batch in plan["batches"]
        for track in batch["tracks"]
        if track["trackId"] in FIRST_BATCH_TRACKS
    }
