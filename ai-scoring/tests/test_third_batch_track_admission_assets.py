...
def test_third_batch_admission_assets_exist_and_meet_sample_mix_gate():
    for track_id in THIRD_BATCH_TRACKS:
...
def test_third_batch_admission_assets_pass_candidate_gate():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    for track_id, rule_file in THIRD_BATCH_TRACKS.items():
...
def test_third_batch_plan_entries_reference_admission_assets_as_pilot_generated_only():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    planned_tracks = {
        track["trackId"]: track
        for batch in plan["batches"]
        for track in batch["tracks"]
        if track["trackId"] in THIRD_BATCH_TRACKS
    }
