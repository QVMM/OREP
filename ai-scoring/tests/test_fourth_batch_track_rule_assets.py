...
def test_fourth_batch_rules_are_promoted_to_pilot_but_not_active():
    for file_name in FOURTH_BATCH_NEW_RULES.values():
...
def test_fourth_batch_rule_hashes_match_canonical_payloads():
    for file_name in FOURTH_BATCH_NEW_RULES.values():
        rule = load_rule(file_name)
...
def test_fourth_batch_rules_pass_candidate_gate_with_ready_assets():
...
def test_fourth_batch_plan_entries_reference_generated_draft_rules():
    plan = load_track_expansion_plan(RUBRIC_DIR / "track_expansion_plan_v1.json")
    planned_tracks = {
        track["trackId"]: track
        for batch in plan["batches"]
        for track in batch["tracks"]
        if track["trackId"] in FOURTH_BATCH_NEW_RULES
    }

    assert set(planned_tracks) == set(FOURTH_BATCH_NEW_RULES)
    for track_id, track in planned_tracks.items():
        ...
        assert track["structuredRuleFile"].endswith(FOURTH_BATCH_NEW_RULES[track_id])
