...
class VideoAnalysisService:
    ...
    def __init__(self):
        ...
        self.batch_size = settings.VIDEO_BATCH_SIZE  # 8
...
不要输出JSON之外的任何文字。"""
    ...
    def _analyze_batch_with_recovery(
        ...
        """Analyze a batch; if the batch fails, retry frame-by-frame so errors are diagnosable."""
        total_tokens = {"prompt_tokens": 0, "completion_tokens": 0}
        try:
            return self._analyze_batch(batch, start_index, timestamps_sec)
        except Exception as e:
            logger.error(f"帧 {start_index + 1}-{start_index + len(batch)} 批量分析失败，拆分单帧重试: {e}")
