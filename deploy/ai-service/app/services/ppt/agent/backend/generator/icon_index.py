...
class IconIndex:
    ...
    def _embed_query(self, query: str):
        ...
        api_key = (
            ...
            or (os.getenv("GOOGLE_GENAI_API_KEY") or "").strip()
        )
        if not api_key:
            # Lexical search still works without embeddings; avoid noisy stack traces.
            return None
        try:
            genai, types = _genai()
            ...
            logger.warning("Failed to embed icon query (falling back to lexical search): %s", query[:120])
            return None
