...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(
        ...
        if normalized_base_url and "xiaomimimo.com" in normalized_base_url:
            # MiMo token-plan is reachable directly in local dev, but exported
            # HTTP/SOCKS proxies can break TLS tunneling and surface as vague
            # "Connection error" failures.  Keep this scoped to MiMo so other
            # providers still respect the user's proxy settings.
            self._http_client = httpx.AsyncClient(trust_env=False)
            client_kwargs["http_client"] = self._http_client
        # Disable the SDK's very short built-in retry loop.  Long PPT jobs need
        # the shared retry helper's slower cooldown instead of several rapid
        # retries that can hammer a temporarily unhealthy upstream gateway.
    ...
    def _is_qwen_request(self, model: str | None = None) -> bool:
        normalized = (model or "").lower()
        return (
            self._provider_name == "qwen"
            or "dashscope.aliyuncs.com" in self._base_url
            or normalized.startswith("qwen")
        )

    def _is_mimo_request(self, model: str | None = None) -> bool:
        normalized = (model or "").lower()
        return (
            self._provider_name == "mimo"
            or "xiaomimimo.com" in self._base_url
            or normalized.startswith("mimo")
        )

    def _normalize_max_tokens(self, model: str, max_tokens: int | None) -> int | None:
        if max_tokens and self._is_mimo_request(model):
    ...
    def _build_chat_kwargs(
        ...
        *,
        ...
        if self._is_qwen_request(model):
            kwargs.setdefault("extra_body", {})
            kwargs["extra_body"].setdefault("enable_thinking", False)
        if self._is_mimo_request(model):
            kwargs.setdefault("extra_body", {})
            kwargs["extra_body"].setdefault("enable_thinking", False)
        if stream:
            kwargs["stream"] = True
    ...
    async def chat(
        ...
        *,
        ...
        if resp.usage:
            ...
            provider_name = self._provider_name if self._provider_name else "openai"
