...
# Default retry budget is intentionally configurable because long PPT runs
# magnify otherwise harmless upstream/network blips.
DEFAULT_MAX_ATTEMPTS = int(os.getenv("PPT_LLM_RETRY_ATTEMPTS", "6") or "6")
DEFAULT_BASE_DELAY = float(os.getenv("PPT_LLM_RETRY_BASE_DELAY", "1.0") or "1.0")
DEFAULT_MAX_DELAY = float(os.getenv("PPT_LLM_RETRY_MAX_DELAY", "20.0") or "20.0")
...
def _is_retryable(exc: BaseException) -> bool:
    ...
    if any(tok in name for tok in ("timeout", "connection", "apiconnection", "protocol")):
        return True
    text = str(exc).lower()
    if any(
        marker in text
