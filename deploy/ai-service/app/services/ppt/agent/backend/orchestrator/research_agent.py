...
def _is_mimo_anthropic_provider(llm: LLMProvider) -> bool:
    provider_name = str(getattr(llm, "_provider_name", "") or "").lower()
    base_url = str(getattr(llm, "_base_url", "") or "").lower()
    return provider_name == "mimo" and "anthropic" in base_url
...
def _is_competition_page_target(num_pages: int | None) -> bool:
    return bool(num_pages and 35 <= num_pages <= 45)


def _replace_or_insert_page_type(page: str, page_type: str) -> str:
...
def _normalize_competition_manuscript_structure(
    manuscript: str,
    num_pages: int | None,
    detail_level: str = "normal",
) -> tuple[str, str | None]:
    """Repair small metadata drift locally for 35-45 page competition decks.

    Rewriting a full 38-page manuscript because one chapter marker drifted is
    fragile and slow. If the slide count is already in the competition range,
    normalize page_type metadata deterministically and let the single-page SVG
    executor proceed.
    """
    if not _is_competition_page_target(num_pages):
        return manuscript, None

    pages = split_manuscript_pages(manuscript)
    if not 35 <= len(pages) <= 45:
        return manuscript, None

    budget = page_type_budget(num_pages, detail_level)
    target_chapters = max(0, budget.get("chapter", 0))
    normalized: list[str] = []
    chapter_count = 0
    changed: list[str] = []
    ...
    for index, page in enumerate(pages, start=1):
        ...
        if index == 1:
            ...
            desired = "ending"
        elif page_type == "chapter" and chapter_count < target_chapters:
            desired = "chapter"
            chapter_count += 1
        else:
            desired = "content"

        if page_type != desired or "page_type" not in page:
            changed.append(f"{index}:{page_type}->{desired}")
            page = _replace_or_insert_page_type(page, desired)
        normalized.append(page)

    if not changed:
        return manuscript, None
    return "\n\n---\n\n".join(normalized), "normalized page_type metadata (" + ", ".join(changed[:12]) + ")"
...
def _manuscript_structure_error(
    ...
    if num_pages:
        ...
        if _is_competition_page_target(num_pages):
            if not 35 <= len(pages) <= 45:
                return f"expected 35-45 competition slides, got {len(pages)}"
        ...
        if drift and not _is_competition_page_target(num_pages):
...
async def analyze_paper(
    ...
    *,
    ...
    for attempt in range(1, MAX_MANUSCRIPT_ATTEMPTS + 1):
        ...
        normalized_manuscript, normalization_note = _normalize_competition_manuscript_structure(
            manuscript,
            num_pages,
            detail_level,
        )
        if normalization_note:
            logger.info("Research Pass 3 local normalization: %s", normalization_note)
    ...
    if _is_mimo_anthropic_provider(llm):
        final_error = _manuscript_validation_error(manuscript, paper, num_pages, detail_level)
        if final_error:
            logger.warning(
                "Skipping provider self-review pass, keeping generated manuscript with validation warning: %s",
                final_error,
            )
        else:
            logger.info("Skipping provider self-review pass after valid manuscript")
        _debug_write_text(debug_dir, "research_final_manuscript.md", manuscript)
        if on_progress:
            on_progress("质量复核已完成，继续生成页面内容", 0.28)
        return manuscript
