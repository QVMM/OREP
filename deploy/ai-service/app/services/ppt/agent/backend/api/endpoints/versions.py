...
def _resolve_project_dir(job_id: str, request: Request) -> Path:
...
async def list_versions(job_id: str, request: Request) -> VersionsResponse:
    job = session_manager.get_job(job_id)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found.")
    ensure_job_owner(job, request)
    ...
    try:
        project_dir = _resolve_project_dir(job_id, request)
...
async def get_version(job_id: str, round_name: str, request: Request) -> VersionDetailResponse:
    project_dir = _resolve_project_dir(job_id, request)
...
async def delete_version(job_id: str, round_name: str, request: Request) -> dict[str, Any]:
    project_dir = _resolve_project_dir(job_id, request)
    round_dir = project_dir / "svg_archive" / round_name
    if not round_dir.exists() or not round_dir.is_dir():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Version not found.")
