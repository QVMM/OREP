...
async def search_images_endpoint(
    job_id: str, http_request: Request, request: ImageSearchRequest
) -> ImageSearchResponse:
    """Search for images online and return results (no download)."""
    job = session_manager.get_job(job_id)
    ensure_job_owner(job, http_request)
    ...
    if not config.tavily_api_key and not config.serpapi_key:
        from backend.orchestrator.qwen_image import (
            generate_qwen_image_asset,
            qwen_image_configured,
        )
        if not qwen_image_configured():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No Tavily/SerpAPI key or Qwen-Image key configured.",
            )
        asset = await generate_qwen_image_asset(
            request.query,
            Path(job.project_dir) / "images",
        )
        if asset is None:
            raise HTTPException(
...
@router.get("/image-search/{job_id}/asset/{filename}", name="get_generated_image_asset")
async def get_generated_image_asset(job_id: str, filename: str, request: Request) -> FileResponse:
    job = session_manager.get_job(job_id)
    ensure_job_owner(job, request)
    if not job.project_dir:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found.")
    images_dir = (Path(job.project_dir) / "images").resolve()
...
async def apply_image_endpoint(
    job_id: str, payload: ImageApplyRequest, request: Request
) -> ImageApplyResponse:
    ...
    url_hash = hashlib.md5(payload.image_url.encode()).hexdigest()[:8]
    filename = f"search_img_slide{payload.slide_index}_{url_hash}.png"
    output_path = images_dir / filename

    # Download the image
    from backend.orchestrator.image_search import download_image

    saved = await download_image(payload.image_url, output_path)
    if saved is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    svg_path = _find_slide_svg(project_dir, payload.slide_index)
    if svg_path is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Could not find SVG for slide {payload.slide_index}.",
        )
    ...
    if svg_has_image_elements(svg_content):
        ...
        modified = replace_image_in_svg(
            svg_content, new_href, payload.target_element
        )
        action = "replaced"
    else:
        # No <image> element — use LLM to insert, or fallback heuristic
        provider = payload.provider or job.provider or "openai"
        model = payload.model or job.model_name or "gpt-4o"
        ...
        if api_key:
            modified = await auto_insert_image(
                svg_content,
                new_href,
                image_description=payload.image_description or "",
                image_path=saved,
                provider=provider,
                model=model,
                api_key=api_key,
...
async def undo_image_endpoint(job_id: str, request: Request) -> ImageUndoResponse:
