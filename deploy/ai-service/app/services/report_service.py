...
def _normalize_jury_session_to_package(session: dict) -> dict | None:
    ...
    for r in reports:
        ...
        score_cards.append({
            'code': code,
            'judge_code': code,
            'role_label': r.get('role_label') or r.get('persona_name') or code,
            ...
            'top_concerns': list(concerns)[:3] if concerns else [],
            'focus': _value_text(view.get('rubric_focus') or view.get('optimization_angle') or ''),
            'comment': comment,
            'summary': comment,
...
def _dimension_judgment(dim_key: str, name: str, sc_f: float, mx_f: float, dim: dict, ai_score: dict) -> tuple[str, str]:
    ...
    overview = ai_score.get('score_overview') or {}
    diagnosis = _humanize_dimension_tokens(
        _value_text(overview.get('diagnosis') or ai_score.get('summary') or '')
    )
    issues = _as_text_list(ai_score.get('critical_issues') or overview.get('key_issues'), limit=12)
    aliases = _dim_aliases(dim_key, name)
    action_keys = _dim_action_keys(dim_key, name)

    def _hit(text: str, keys: list[str]) -> bool:
        return any(a and a in (text or '') for a in keys)

    matched_issue = next((t for t in issues if _hit(t, aliases)), '')
    matched_diag = ''
    if diagnosis:
        for sent in re.split(r'(?<=[。！？；\n])', diagnosis):
            if _hit(sent, aliases):
                matched_diag = sent.strip()
                break

    ratio = (sc_f / mx_f) if mx_f > 0 else 0
    if ratio >= 0.75:
        level = '本维相对扎实'
    elif ratio >= 0.45:
        level = '本维有基础，证据仍不足'
    ...
    if matched_issue:
        judgment = f'{level}。关键缺口：{matched_issue}'
    elif matched_diag:
        judgment = f'{level}。{matched_diag}'
    else:
        judgment = f'{level}（{sc_f:g}/{mx_f:g}）。'

    tip = ''
    best_score = -1
    for item in (ai_score.get('action_plan') or []):
        if not isinstance(item, dict):
            continue
        title = _value_text(item.get('title') or '')
        problem = _value_text(item.get('problem') or '')
        method = _value_text(item.get('method') or item.get('action') or '')
        # 严格：标题或问题必须命中本维专属键
        score = 0
        if _hit(title, action_keys):
            score += 3
        if _hit(problem, action_keys):
            score += 2
        if score <= 0:
            continue
        # 取 method 摘要；无 method 则用 title
        candidate = method if len(method) > 12 else (title if len(title) > 4 else problem)
        if candidate and score > best_score:
            tip = candidate
            best_score = score
    return _humanize_dimension_tokens(judgment), _summarize_improvement(tip)
...
def _humanize_dimension_tokens(text: str) -> str:
    ...
    # 模型口语冗余：「技能熟练度（技能水平）」→「技能水平」
    s = re.sub(r'技能熟练度（技能水平）', '技能水平', s)
    s = re.sub(r'专业性（职业素养）', '职业素养', s)
    s = re.sub(r'创新性（创新能力）', '创新能力', s)
    s = re.sub(r'技能熟练度', '技能水平', s)
    return s
...
def generate_report(result: dict, output_dir: str) -> str:
    """从评分结果 JSON 生成 PDF 文档。

    叙事：封面 → 总览 → 证据摘要 → 维度 → 语音 → 亮点/追问 →
    （可选真实评审团）→ 融合 → 结构 → 行动 → 团队 → 画像 → 结论。
    章节号按实际渲染顺序生成；未开评委绝不输出 J0x 人格。
    """
    if not isinstance(result, dict):
