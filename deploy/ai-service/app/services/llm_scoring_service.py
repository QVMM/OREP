...
## 深度分析要求（内部执行，输出为结构化结论）
- 评分总览：输出一句总诊断、亮点标签和关键问题标签，必须能解释综合得分为什么落在当前区间。
- 证据审计：把亮点、扣分项和建议分为“强证据、弱证据、缺失证据、矛盾证据”；强证据必须带时间点/原文/画面摘要之一。
- 评委可能追问：输出 3–6 条「评委可能追问」问题清单（judge_questioning），必须指向真实证据缺口；禁止虚构评委人格、禁止输出九位 J0x/MBTI 评委独立打分表。多评委评审团由产品单独开关触发，不在本评分 JSON 中伪造。
- 音视频融合：判断“说的内容”和“现场呈现”是否一致，列出矛盾点；没有视觉数据时也要基于语音节奏和转写证据输出保守结论，不得编造画面。
- 结构对标：按“破冰钩子、问题共鸣、方案展示、证据验证、商业闭环、团队致胜、有力收尾”评估路演叙事，指出最影响得分的结构断点。
- 行动计划：按 P0/P1/P2 输出，必须包含问题、做法、负责人角色、时间建议、验收标准；不要只写“加强”“优化”。
- 团队/角色优化：按主讲、技术演示、数据/市场、运营/协作等角色给训练路线；无法识别具体人名时用角色名。
- 最终结论：用可辩护的语言总结三类内容：站得住脚的优势、最关键扣分项、下一轮最优先补齐的证据；标题使用“综合判定”，不要写“九评委综合评价”。
...
OUTPUT_FORMAT = """{
  ...
  "score_overview": {
    "diagnosis": "一句话总诊断，解释本轮得分的核心原因",
    "highlights": ["亮点标签1", "亮点标签2"],
    "key_issues": ["关键问题标签1", "关键问题标签2"]
  },
  "dimensions": {
    "skill_level": {
      "name": "技能水平",
      ...
      "score": 实际得分,
      "items": [
        {"name": "操作规范性", "max_score": 10, "score": 得分(2位小数), "reason": "评分理由，必须引用路演原文或视频帧内容", "improvement": "具体改进建议"},
        {"name": "技能熟练度", "max_score": 15, "score": 得分(2位小数), "reason": "理由", "improvement": "建议"},
  ...
  "highlights": ["亮点1（引用原文或视频帧）", "亮点2"],
  "critical_issues": ["严重问题1（引用原文或视频帧）", "严重问题2"],
  "evidence_summary": {
  ...
  "evidence_audit": [
    {"level": "强证据|弱证据|缺失证据|矛盾证据", "claim": "结论或扣分点", "source": "时间点/原文/画面摘要", "impact": "影响的评分维度或观测点"}
  ],
  "judge_questioning": [
    {"focus": "关注角度", "question": "评委可能追问的问题", "why_it_matters": "为何影响得分", "prep_evidence": "应准备的证据"}
  ],
  "audio_visual_fusion": {
    "summary": "声音表现与现场呈现是否一致的综合判断",
    "contradictions": [
      {"time": "时间点或阶段", "description": "矛盾描述", "audio_score": 0-10或null, "visual_score": 0-10或null, "gap": "差距说明"}
    ],
    "metrics": [
  ...
  "pitch_structure_benchmark": [
    {"stage": "破冰钩子|问题共鸣|方案展示|证据验证|商业闭环|团队致胜|有力收尾", "time_range": "对应时间段或阶段", "score": 0-10, "actual": "实际表现", "gap": "主要差距", "fix": "改进方向"}
  ],
  "action_plan": [
    {"priority": "P0|P1|P2", "title": "行动标题", "problem": "要解决的问题", "method": "具体做法", "owner": "负责角色", "timebox": "时间建议", "acceptance": "验收标准", "observation_codes": ["对应观测点代码，无法确定时为空数组"], "criterion_codes": ["对应评分规则代码，无法确定时为空数组"], "dimension_code": "对应评分维度代码，无法确定时为空", "source_issue_key": "对应问题键，无法确定时为空", "impact_type": "deduction_recovery|evidence_unlock|performance_improvement|unlinked"}
  ],
  "team_optimization": [
    {"priority": "P0|P1|P2", "role": "主讲人A|技术讲解B|数据分析C|全体选手等", "issue": "角色短板", "training": "训练动作", "target": "验收目标"}
  ],
  "final_verdict": {
...
def _backfill_reference_report_sections(result: dict) -> None:
    highlights = _string_list(result.get('highlights')) or ["已完成完整路演展示"]
    issues = _string_list(result.get('critical_issues')) or ["关键证据仍需进一步补齐"]
    priorities = [item for item in result.get('improvement_priorities', []) if isinstance(item, dict)]
    dimensions = result.get('dimensions') or {}

    result.setdefault('score_overview', {
        "diagnosis": _value_or_default(result.get('summary'), f"{highlights[0]}，但{issues[0]}。"),
        "highlights": highlights[:6],
        "key_issues": issues[:6],
    })

    # 严禁补全假评委（jury_review）；多评委仅由 jury_enabled 流水线产出
    result.pop('jury_review', None)

    result.setdefault('audio_visual_fusion', {
        "summary": "未获得完整音视频融合结构时，先按语音节奏和评分证据给出保守复盘。",
        "contradictions": [
            {"time": "复盘阶段", "description": issues[0], "audio_score": None, "visual_score": None, "gap": "需补充现场呈现证据"}
        ],
        "metrics": [
            {"name": "表达节奏", "average": "需复盘", "low": issues[0], "high": highlights[0], "trend": "待优化"}
        ],
    })

    if not result.get('pitch_structure_benchmark'):
        result['pitch_structure_benchmark'] = [
            {"stage": "破冰钩子", "time_range": "开场", "score": 5, "actual": "开场信息已交代，但记忆点需要复核。", "gap": "第一印象证据不足", "fix": "用痛点数据或场景开场。"},
            {"stage": "证据验证", "time_range": "主体", "score": 5, "actual": issues[0], "gap": "证据链仍需补齐", "fix": "补充可追溯数据、截图或现场演示证据。"},
            {"stage": "商业闭环", "time_range": "后段", "score": 5, "actual": "商业模式、目标客户或推广路径需要复核。", "gap": "可持续性判断依据不足", "fix": "补充盈利方式、客户对象和市场容量。"},
        ]

    if not result.get('action_plan'):
        actions = []
        for idx, item in enumerate(priorities[:6], 1):
            actions.append({
                "priority": "P0" if idx <= 2 else "P1",
                "title": item.get('issue') or item.get('dimension') or f"改进项{idx}",
                "problem": item.get('issue') or issues[(idx - 1) % len(issues)],
                "method": item.get('suggestion') or "补充证据并重排讲解结构。",
                "owner": "对应负责选手",
                "timebox": "下一轮路演前",
        ...
        if not actions:
            actions = [{
                ...
                "title": "补齐关键证据",
                "problem": issues[0],
                "method": "为每个关键结论补充数据来源、截图或演示记录。",
                "owner": "全体选手",
                "timebox": "下一轮路演前",
                "acceptance": "关键扣分项均有可追溯证据",
            }]
        result['action_plan'] = actions

    if not result.get('team_optimization'):
        result['team_optimization'] = [
            {"priority": "P0", "role": "主讲人", "issue": "总述与收束需要更聚焦。", "training": "用30秒讲清项目价值，再用证据页承接。", "target": "开场与结尾能被复述。"},
            {"priority": "P0", "role": "技术讲解", "issue": issues[0], "training": "只讲关键技术路径，减少逐行解释。", "target": "评委能听懂技术价值和验证方式。"},
            {"priority": "P1", "role": "全体选手", "issue": "临场协作需要预案。", "training": "彩排故障切换和队友补位话术。", "target": "突发情况10秒内完成过渡。"},
        ]

    if not result.get('final_verdict'):
        result['final_verdict'] = {
            "summary": result.get('score_overview', {}).get('diagnosis', ''),
            "jury_summary": f"项目具备{highlights[0]}等优势，但{issues[0]}仍是下一轮重点。",
            "defensible_strengths": highlights[:3],
            "critical_deductions": issues[:3],
            "next_round_focus": [item.get('suggestion') for item in priorities[:3] if item.get('suggestion')] or issues[:2],
            "priority_actions": [
                {"priority": plan.get('priority', 'P0'), "item": plan.get('title', ''), "owner": plan.get('owner', ''), "acceptance": plan.get('acceptance', '')}
                for plan in result.get('action_plan', [])[:5]
            ],
        }
