...
本方案当前不按“完整商业版一次性交付”立项，而按“可验证试点版”进入研发。

准入判断：

| 维度 | 当前判断 | 处理方式 |
|---|---|---|
| 产品方向 | 成立 | 保留“教学成长 + 竞赛拔尖双轮驱动”定位 |
| 系统融合 | 基本成立 | 继续复用现有会议、评分、录制、PPT、讲稿模块 |
| P0 范围 | 偏大 | 拆成 P0a/P0b/P0c/P0d 四个薄切片 |
| 学校采购价值 | 需要前置验证 | P0 就要提供试点价值页和可导出简报 |
| 能力画像可信度 | 需要校准 | 增加证据等级、老师修正、证据不足状态 |
| 全量开发 | 暂不建议 | P0 试点指标达标后再进入 P1-P4 连续排期 |

### 1.2 试点默认假设

以下假设用于第一版研发排期，试点前需要由学校或交付团队确认：

```text
试点范围：1 个学校 / 1 个专业 / 1-2 个班级 / 1-2 门课程
试点周期：4 周
试点对象：普通学生为主，候选学生少量验证，参赛队伍只验证入口和数据关系
数据口径：第一版能力画像是教学辅助判断，不作为正式评价结果
AI 评分：第一版采用规则 + AI 辅助解释，不做黑箱自动定论
```

### 1.3 试点成功指标

P0 不是以“页面做完”为成功，而是以“学校、老师、学生愿意继续用”为成功。

| 指标 | 目标 | 时间 | 决策意义 |
|---|---:|---|---|
| 学生画像覆盖率 | 试点学生 80% 以上生成画像 | 第 4 周 | 证明平台能覆盖全体学生 |
| 周练习活跃率 | 试点学生 65% 以上每周至少完成 1 次练习 | 第 4 周 | 证明普通学生能高频使用 |
| 单次练习完成率 | 已开始练习中 70% 以上完成 | 第 2 周起 | 判断 AI 练习是否过重 |
| 老师报告查看频次 | 每位试点老师每周查看 2 次以上 | 第 4 周 | 判断教师端是否有日常价值 |
| 问题采纳率 | 老师采纳系统建议问题 50% 以上 | 第 4 周 | 判断问题分流是否可信 |
| 候选推荐认可率 | 老师认可候选推荐 60% 以上 | P2 试点 | 判断画像是否能支撑竞赛选人 |
| 学校简报可用性 | 至少导出 1 份可汇报试点简报 | 第 4 周 | 判断是否具备采购沟通材料 |
...
```text
作为普通学生，我希望每天打开 OREP 后只看到今天最该完成的练习，
以便我不用理解整个平台，也能知道自己该学什么、哪里变强了。

验收：
- 首页主任务只能有 1 个。
- 完成练习后必须展示本次能力变化和下一步建议。
- 无竞赛身份时，不强推参赛项目空间。
```
...
```text
作为竞赛候选学生，我希望看到自己为什么被推荐、距离入队差什么，
以便我能通过专项补练把差距补上，而不是只看到一个抽象分数。
...
```text
作为参赛学生，我希望 OREP 把 PPT、讲稿、模拟展示、复盘和整改任务串起来，
以便我每天知道自己负责哪一项训练，而不是在多个工具里找任务。

验收：
- 竞赛首页主任务优先展示下一场模拟展示或最近整改任务。
- PPT/讲稿/会议必须能绑定同一个竞赛项目。
- 会后问题必须能转成训练任务。
```
...
```text
作为任课老师或指导老师，我希望系统自动汇总学生卡点和候选证据，
以便我用较少时间判断哪些问题要回课堂、哪些学生要专项训练、哪些队员要整改。

验收：
- 教师首页必须展示完成率、高频卡点、预警、候选变化。
- 问题审核单条操作目标控制在 3 分钟内。
- 系统建议必须允许老师采纳、修改、忽略，并记录原因。
```

#### 学校领导

```text
作为学校领导，我希望看到覆盖学生、能力变化、教学改进、竞赛储备和训练成果的证据链，
以便判断 OREP 是否值得采购、续费和推广到更多专业。

验收：
- P0 试点就要有学校试点价值页。
- 学校端默认只看聚合数据，不看学生原始对话。
- 报告必须能导出覆盖率、活跃率、能力趋势、问题回流、候选储备。
```
...
CREATE TABLE IF NOT EXISTS student_ability_profile (
  ...
  evidence_level VARCHAR(32) NOT NULL DEFAULT 'INSUFFICIENT' COMMENT 'INSUFFICIENT/OBSERVING/TRUSTED/STALE',
  valid_practice_count INT NOT NULL DEFAULT 0,
  dimension_coverage_count INT NOT NULL DEFAULT 0,
  ...
  KEY idx_evidence_level (tenant_id, evidence_level)
...
CREATE TABLE IF NOT EXISTS competition_candidate (
  ...
  risk_json JSON NULL,
  gap_json JSON NULL,
  evidence_level VARCHAR(32) NOT NULL DEFAULT 'INSUFFICIENT' COMMENT '推荐时画像证据等级',
...
P0 不做“大而全首页 + 全量画像 + 完整教师工作台”。P0 的目标是用最少功能证明三件事：
...
P0 拆为四个薄切片，每个切片都必须能独立演示、独立验收、独立回滚。
...
→ practice_task
→ PracticeSession
→ dialogue_practice / dialogue_detail
→ ability_event
...
```text
Dashboard.vue 只改普通学生主任务区
PracticeTaskList.vue 基础版
PracticeSession.vue 基础版
...
```text
TeachingOutlineController/Service 基础创建和发布
PracticeController/Service
AbilityEventService
AbilityProfileController/Service
DashboardController/Service 的学生分支
```
...
```text
老师能创建 1 个课程知识点并发布为练习任务
普通学生首页只显示 1 个今日主任务
学生能完成至少 4 层对话：事实 / 推理 / 应用 / 复盘
每轮回答写入 dialogue_detail
完成练习后生成 ability_event
student_ability_profile 被更新
学生结果页能看到本次能力变化、证据数量、下一步建议
无画像数据时 AbilityProfile 显示明确空状态
```
...
Dashboard.vue 老师身份主任务区
PracticeTaskList.vue 支持老师查看任务完成摘要
```

#### 后端范围

```text
TeacherWorkbenchController/Service 基础版
PracticeService 增加班级完成率统计
AbilityProfileService 增加班级维度聚合
```
...
先不做完整 IssueEventCenter，把问题回流做成教师工作台中的轻量流程：
...
```text
TeacherWorkbench.vue 增加“待回流问题”区块
PracticeTaskList.vue 增加补练 Tab
Dashboard.vue 学生主任务支持补练优先级
```

#### 后端范围

```text
IssueEventController/Service 基础列表和审核
IssueRoutingService 规则版
PracticeService 增加补练任务生成与查询
```
...
```text
系统能从练习结果生成 issue_event
issue_event 默认不直接发给学生
老师能采纳、修改、忽略问题建议
老师采纳问题后能创建全班补练或个人补练
学生首页能优先看到逾期补练
补练完成后继续生成 ability_event
```
...
不等 P4 才证明学校采购价值。P0 就提供一个轻量试点价值页，服务校领导、教务、项目负责人汇报。

#### 前端范围

```text
SchoolDashboard.vue 试点价值基础版
Statistics.vue 增加“试点概览”入口
ReportExport 基础入口
```
...
```text
SchoolDashboardController/Service 基础聚合
ReportExportController/Service 基础 PDF/HTML 任务记录
```
...
P0 只有同时满足功能验收和试点指标，才允许进入 P1/P2 连续排期。

| 通过项 | 标准 |
|---|---|
| 功能闭环 | P0a-P0d 核心链路全部跑通 |
| 学生侧 | 试点学生画像覆盖率达到 80% |
| 老师侧 | 试点老师每周至少查看 2 次班级概览 |
| 学校侧 | 能导出试点简报，且数据口径可解释 |
| 数据侧 | 画像必须显示证据等级，不允许无证据给强结论 |
| 体验侧 | 普通学生首页只保留一个主任务，不做多入口堆叠 |
...
P2 的前置条件是：P0 画像证据达到可信等级，且老师能理解画像来源。否则不启动自动候选推荐，只展示“证据不足”。
...
PPT、讲稿、会议、复盘必须能绑定同一个 projectId
参赛学生首页必须优先展示下一场模拟展示或最近整改任务
```
...
学校能导出至少 3 类报告
学校端不展示学生原始对话
学校能按专业、班级、课程、竞赛项目查看聚合数据
学校报告能追溯到“练习数据 → 画像变化 → 教学回流 → 竞赛储备”的证据链
```
...
能力画像第一版不能被包装成“绝对评价”。它是教学辅助数据，必须展示证据等级。

#### 可信度状态

| 状态 | 触发条件 | 页面展示 | 是否可用于候选推荐 |
|---|---|---|---|
| INSUFFICIENT | 有效练习少于 3 次，或近 30 天无数据 | 证据不足 | 不可强推荐 |
| OBSERVING | 有 3 次以上练习，但维度证据不均衡 | 观察中 | 只可进入观察池 |
| TRUSTED | 近 30 天有 5 次以上有效练习，且至少覆盖 3 个维度 | 可信画像 | 可用于候选推荐 |
| STALE | 最近一次能力事件超过 14 天 | 需要更新 | 推荐先补练 |

#### 有效证据定义

一次练习只有满足以下条件，才进入画像计算：

```text
完成事实 / 推理 / 应用 / 复盘中至少 3 层
总回答字数达到最低要求
非空回答比例 100%
templateRiskScore 低于阈值，或经过追问后通过
练习状态为 FINISHED
```
...
```text
AbilityProfile.vue 每个能力分必须显示证据数量和最近更新时间
TeacherWorkbench.vue 班级报告必须显示“证据不足学生数”
CompetitionCandidate.vue 推荐候选必须显示证据等级
SchoolDashboard.vue 只展示聚合画像覆盖率，不展示原始对话
```

### 10.5 老师修正与异议机制

老师可以修正系统判断，但不能直接篡改学生原始数据。
...
```text
直接改 dialogue_detail 原始回答
直接改 ability_event 原始分数
删除学生练习证据
学校管理员查看学生原始对话全文
```
...
```text
所有老师修正写入 review_note / reviewed_by / reviewed_at
候选推荐调整写入 competition_candidate.reviewed_by / reviewed_at
后续可以新增 teacher_override_log，P0 暂不新增，先复用审核字段
```
...
```text
普通学生首页是否只突出 1 个主任务
练习结束页是否显示能力变化、证据等级、下一步建议
无画像学生是否显示“完成一次课程练习后生成画像”
教师工作台是否能在首屏看到完成率、高频卡点、预警学生
学校试点价值页是否能展示覆盖率、活跃率、画像生成率、Top 卡点
```
...
student_ability_profile.evidence_level 被正确更新
证据不足学生不得进入强推荐候选列表
```

### 11.4 试点成功指标验收

P0 试点结束后，交付团队需要按以下口径输出结果：

| 指标 | 数据来源 | 通过标准 |
|---|---|---|
| 学生画像覆盖率 | student_ability_profile / users | >= 80% |
| 周练习活跃率 | dialogue_practice.finished_at | >= 65% |
| 单次练习完成率 | dialogue_practice.status | >= 70% |
| 老师报告查看频次 | 后续埋点或访问日志 | 每位老师每周 >= 2 次 |
| 问题采纳率 | issue_event.review_status | >= 50% |
| 试点简报可导出 | report_export_task | 至少 1 份 |

如果试点指标未达标，不进入 P2/P3 扩展，先回到 P0 优化练习体验、教师工作台或画像解释。
...
```text
1. P0a 数据表、Entity、Mapper：teaching_outline / practice_task / dialogue_* / ability_event / student_ability_profile
2. P0a PracticeController + PracticeService：开始、回答、结束练习
3. P0a AbilityEventService + AbilityProfileService：画像计算和证据等级
4. P0a PracticeSession.vue + AbilityProfile.vue：学生完成练习并看到画像
5. P0a Dashboard.vue 普通学生主任务区：只展示今日最重要任务
6. P0b TeacherWorkbench.vue 基础版：班级完成率、高频卡点、预警学生
7. P0c issue_event / remedial_task + 轻量回流审核
8. P0d SchoolDashboard.vue 试点价值页 + report_export_task
9. P1 IssueEventCenter.vue 完整版 + 批量问题处理
10. P2 CompetitionCandidateService + 候选推荐
11. P3 CompetitionWorkbench.vue + 会议/PPT/讲稿融合
12. P4 SchoolDashboard.vue 完整驾驶舱 + 报告导出
```
...
```json
{
  ...
  "profileSnapshot": {
    ...
    "applicationScore": 72,
    "expressionScore": 76,
    "evidenceLevel": "OBSERVING",
    "validPracticeCount": 4,
    "dimensionCoverageCount": 3
  },
  "nextTasks": [
    {
      "id": 1008,
...
```json
{
  ...
  "evidenceLevel": "OBSERVING",
  "evidenceLevelLabel": "观察中",
  "validPracticeCount": 4,
  "dimensionCoverageCount": 3,
  "evidenceWarning": "当前画像可用于学习建议，暂不作为强候选推荐依据。",
  "dimensions": [
    {
      "dimension": "PROFESSIONAL",
      "label": "专业能力",
  ...
  "competitionFit": {
    ...
    "status": "NOT_IN_POOL",
    "evidenceLevel": "OBSERVING",
    "recommendationStrength": "WEAK_OBSERVE",
    "gap": [
...
private String resolveEvidenceLevel(Long tenantId, Long studentId, List<AbilityEvent> events);
```
...
```text
validPracticeCount < 3 → INSUFFICIENT
validPracticeCount >= 3 且 dimensionCoverageCount < 3 → OBSERVING
validPracticeCount >= 5 且 dimensionCoverageCount >= 3 且 lastEventAt <= 14 天 → TRUSTED
lastEventAt > 14 天 → STALE
```

注意：

```text
evidence_level 影响页面解释和候选推荐强度。
evidence_level 不直接影响原始 ability_event 分数。
老师可以对画像添加备注，但不能改原始事件。
```
...
```text
evidence_level=TRUSTED 且 fit_score >= 85 → STRONG_RECOMMEND
evidence_level=OBSERVING 且 fit_score >= 80 → WEAK_OBSERVE
evidence_level=INSUFFICIENT → EVIDENCE_NOT_ENOUGH
evidence_level=STALE → NEED_REFRESH_PRACTICE
```

只有 `STRONG_RECOMMEND` 可以进入强推荐列表；`WEAK_OBSERVE` 只能进入观察池。
